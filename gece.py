  ...
  if event.is_private:
    async for usr in client.iter_participants(event.chat_id):
     ...
     await event.reply(f"{ad} {startmesaj}", buttons=(
                      ...
                      [Button.url('🌱 Məni Qrupa Əlavə Et', f'https://t.me/{JasmineTaggerBot}?startgroup=a')],
...
async def handler(event):
    async for usr in client.iter_participants(event.chat_id):
     ...
     await event.edit(f"{ad} {startmesaj}", buttons=(
                      ...
                      [Button.url('⚕️ Məni Qrupa Əlavə Et', f'https://t.me/{JasmineTaggerBot}?startgroup=a')],
                     [Button.url('🇹🇷 Söhbət Qrupu', f'https://t.me/{Alay57Chat}')],
...
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    await client.send_message(event.chat_id, "❄️ Tağ Başladı\n⏱️ İnterval - 2 saniyə",
                    buttons=(
                      [
                      Button.url('📣 Support', f'https://t.me/{JasmineSup}')
                      ]
                    )
                  ) 
